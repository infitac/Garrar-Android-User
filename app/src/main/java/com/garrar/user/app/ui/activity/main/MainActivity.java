...
import com.garrar.user.app.ui.fragment.view_courier_detail.ViewCourierDetailFragment;
...
import com.garrar.user.app.chat.ChatActivity;
import com.garrar.user.app.common.Constants;
import com.garrar.user.app.common.InfoWindowData;
import com.garrar.user.app.common.LocaleHelper;
import com.garrar.user.app.common.PolyUtil;
...
import com.garrar.user.app.data.network.model.LatLngFireBaseDB;
...
import com.garrar.user.app.ui.activity.coupon.CouponActivity;
import com.garrar.user.app.ui.activity.help.HelpActivity;
import com.garrar.user.app.ui.activity.invite_friend.InviteFriendActivity;
import com.garrar.user.app.ui.activity.location_pick.LocationPickActivity;
import com.garrar.user.app.ui.activity.notification_manager.NotificationManagerActivity;
import com.garrar.user.app.ui.activity.passbook.WalletHistoryActivity;
import com.garrar.user.app.ui.activity.payment.PaymentActivity;
import com.garrar.user.app.ui.activity.profile.ProfileActivity;
import com.garrar.user.app.ui.activity.setting.SettingsActivity;
import com.garrar.user.app.ui.activity.wallet.WalletActivity;
import com.garrar.user.app.ui.activity.your_trips.YourTripActivity;
import com.garrar.user.app.ui.fragment.RateCardFragment;
import com.garrar.user.app.ui.fragment.book_ride.BookRideFragment;
import com.garrar.user.app.ui.fragment.invoice.InvoiceFragment;
import com.garrar.user.app.ui.fragment.rate.RatingDialogFragment;
import com.garrar.user.app.ui.fragment.schedule.ScheduleFragment;
import com.garrar.user.app.ui.fragment.searching.SearchingFragment;
import com.garrar.user.app.ui.fragment.service.ServiceTypesFragment;
import com.garrar.user.app.ui.fragment.service_flow.ServiceFlowFragment;
import com.garrar.user.app.utils.AppConstant;
...
import static com.garrar.user.app.common.Constants.Status.ARRIVED;
import static com.garrar.user.app.common.Constants.Status.COMPLETED;
import static com.garrar.user.app.common.Constants.Status.DROPPED;
import static com.garrar.user.app.common.Constants.Status.EMPTY;
import static com.garrar.user.app.common.Constants.Status.PICKED_UP;
import static com.garrar.user.app.common.Constants.Status.RATING;
import static com.garrar.user.app.common.Constants.Status.SEARCHING;
import static com.garrar.user.app.common.Constants.Status.SERVICE;
import static com.garrar.user.app.common.Constants.Status.STARTED;
import static com.garrar.user.app.data.SharedHelper.key.PROFILE_IMG;
import static com.garrar.user.app.data.SharedHelper.key.SOS_NUMBER;
...
public class MainActivity extends BaseActivity implements
    ...
    private Marker srcMarker, destMarker, s_marker1, s_marker2;
...
/*        Intent telrIntent = getIntent();
    ...
    @OnClick({R.id.menu, R.id.gps, R.id.btn_create_task,/*R.id.source,*/ /*R.id.destination,*/ R.id.changeDestination, R.id.ivBack, R.id.llPickHomeAdd, R.id.llPickWorkAdd})
        ...
        if (direction.isOK()) {
            ...
            if (!route.getLegList().isEmpty()) {
                ...
                if(route.getLegList().size() == 2) {
                    ...
                    s_marker1 = mGoogleMap.addMarker(new MarkerOptions()
                            .icon(BitmapDescriptorFactory.fromResource(R.drawable.des_icon))
                            .position(stop));
                }
                if(route.getLegList().size() == 3) {
                    ...
                    s_marker2 = mGoogleMap.addMarker(new MarkerOptions()
                            .icon(BitmapDescriptorFactory.fromResource(R.drawable.des_icon))
                            .position(stop1));
                }
            }
            /*for(int i = 0; i<route.getLegList().size(); i++){
                for(int j = 0; j < route.getLegList().get(i).getDirectionPoint().size(); j++){
                    try {
                        polyLinePoints.addAll(route.getLegList().get(0).getDirectionPoint());
    ...
    public void changeFragment(Fragment fragment) {
        ...
        if (fragment != null) {
            if (fragment instanceof BookRideFragment || fragment instanceof ServiceTypesFragment ||
                    ...
                    fragment instanceof AddCourierDetailFragment || fragment instanceof ViewCourierDetailFragment)
            ...
            if (fragment instanceof RateCardFragment)
                ...
                fragmentTransaction.addToBackStack(fragment.getTag());
            else if(fragment instanceof AddCourierDetailFragment)
                fragmentTransaction.addToBackStack(fragment.getTag());
            else if(fragment instanceof ViewCourierDetailFragment)
                fragmentTransaction.addToBackStack(fragment.getTag());
            try {
    ...
    public void onSuccess(@NonNull User user) {
        ...
        System.out.println("+++++++ user details:" + user.getWalletBalance());
        System.out.println("+++++++ user details:" + user.getCurrency());
...
//        SharedHelper.putKey(this, "currency", "U+20BA");
//        SharedHelper.putKey(this, "currency", "#x20a4;");
//        SharedHelper.putKey(this, "currency", user.getCurrency());
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUEST_PICK_LOCATION) if (resultCode == Activity.RESULT_OK) {
            ...
            if (MvpApplication.RIDE_REQUEST.containsKey(SRC_ADD)
                ...
                CURRENT_STATUS = SERVICE;
                changeFlow(CURRENT_STATUS);
                if (data.hasExtra(SRC_ADD)) {
                    MvpApplication.RIDE_REQUEST.put(SRC_ADD, data.getStringExtra(SRC_ADD));
                    //sourceTxt.setText(String.valueOf(MvpApplication.RIDE_REQUEST.get(SRC_ADD)));
                    MvpApplication.RIDE_REQUEST.put(SRC_LAT, data.getDoubleExtra(SRC_LAT, 0));
                    MvpApplication.RIDE_REQUEST.put(SRC_LONG, data.getDoubleExtra(SRC_LONG, 0));
                    MvpApplication.RIDE_REQUEST.put(DEST_ADD0, data.getStringExtra(SRC_ADD));
                    MvpApplication.RIDE_REQUEST.put(DEST_LAT0, data.getDoubleExtra(SRC_LAT, 0));
                    MvpApplication.RIDE_REQUEST.put(DEST_LONG0, data.getDoubleExtra(SRC_LONG, 0));
                }
                LatLng origin = new LatLng((Double) MvpApplication.RIDE_REQUEST.get(SRC_LAT), (Double) MvpApplication.RIDE_REQUEST.get(SRC_LONG));
                LatLng destination = new LatLng((Double) MvpApplication.RIDE_REQUEST.get(DEST_LAT), (Double) MvpApplication.RIDE_REQUEST.get(DEST_LONG));
                drawRoute(origin, destination);
        ...
        if (isLocationPermissionGranted) {
            ...
            placesClient.findCurrentPlace(request).addOnSuccessListener(((response) -> {
                for (PlaceLikelihood placeLikelihood : response.getPlaceLikelihoods()) {
                    ...
                    MvpApplication.RIDE_REQUEST.put(DEST_ADD0, placeLikelihood.getPlace().getAddress());
                    MvpApplication.RIDE_REQUEST.put(DEST_LAT0, placeLikelihood.getPlace().getLatLng().latitude);
                    MvpApplication.RIDE_REQUEST.put(DEST_LONG0, placeLikelihood.getPlace().getLatLng().longitude);
                    break;
...
//            MvpApplication.isTelr = checkStatusResponse.getTelr() == 1;
            ...
            //SharedHelper.putKey(this, "currency", AppConstant);
//            SharedHelper.putKey(this, "currency", "U+20BA");
//            SharedHelper.putKey(this, "currency", "#x20a4;");
//            SharedHelper.putKey(this, "currency", checkStatusResponse.getCurrency());
    ...
    @Override
    public void onSuccessCard(List<Card> cards) {
        try {
            hideLoading();
        } catch (Exception e1) {
            e1.printStackTrace();
        ...
        for (int i = 0; i < cards.size(); i++) {
            MvpApplication.RIDE_REQUEST.put(Constants.RIDE_REQUEST.LAST_FOUR, cards.get(i).getLastFour());
            MvpApplication.RIDE_REQUEST.put(Constants.RIDE_REQUEST.CARD_ID, cards.get(i).getCardId());
        }

        System.out.println("+++++ home card details: " + MvpApplication.RIDE_REQUEST.get(Constants.RIDE_REQUEST.CARD_ID));
        System.out.println("+++++ home card details: " + MvpApplication.RIDE_REQUEST.get(Constants.RIDE_REQUEST.CARD_LAST_FOUR));
    }
}
